import { Div, Span } from 'htmlmodule'
...
export class TreeItem extends ListItem {
     ...
     */
    get label() {
        const first = this.labelledBy[0]
        return first? first.textContent : ''
    }

    /**
     * @param {NodeAssembler[]} labelledBy
     */
    set labelledBy(labelledBy) {
        if(labelledBy.length) {
            const group = this.find(Group)
            if(group) group.ownerElement.before(...labelledBy)
            else this.append(...labelledBy)
        }
