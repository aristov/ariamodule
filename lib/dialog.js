...
const { getNodeOf, getElementOf } = ARIAWindow
...
export class Dialog extends ARIAWindow {
    ...
    /**
     * @returns {boolean}
     * todo: add trigger to tabbableElements if dialog is non modal
     */
    focusTrigger() {
    ...
    onTabKeyDown(event) {
        ...
        if(elements.length) {
            ...
            if(event.shiftKey) {
                const triggerNode = getNodeOf(getElementOf(this.trigger))
                if(target === triggerNode) {
                    event.preventDefault()
                    last.focus()
                }
            }
            else if(target === last.node) {
                event.preventDefault()
     ...
     */
    set trigger(trigger) {
        trigger = this.getRoleOf(trigger) || this.getInstanceOf(trigger)
        if(trigger) {
