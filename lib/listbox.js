 ...
 * todo scrolling fix
 * todo options.forEach(opt => opt.selected = true)
 * todo orientaion = horizontal
...
export class ListBox extends Select {
    ...
    init(init) {
        ...
        if(selectedOptions.length) {
            this.activeDescendant = selectedOptions[0]
        }
    ...
    scrollTo(option) {
        ...
        const optionBottom = optionNode.offsetTop + optionNode.clientHeight
        const nodeBottom = node.scrollTop + node.clientHeight
        if(optionNode.offsetTop < node.scrollTop || optionBottom > nodeBottom) {
            const delta = optionNode.clientHeight - node.clientHeight
            node.scrollTop = optionNode.offsetTop + Math.floor(delta / 2)
        }
    }
