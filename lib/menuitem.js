...
export class MenuItem extends Command {
    ...
    install(menu) {
        ...
        if(this.menu.orientation === 'horizontal') {
            ...
            menu.style.left = node.offsetLeft + node.offsetWidth + 'px'
        }
        this.after(menu)
    }
    ...
    onArrowUpKeyDown(event) {
        ...
        if(menu.orientation === 'vertical') {
            ...
            const items = this.controls[0].items.filter(({ disabled }) => !disabled)
            this.expanded = true
    ...
    onArrowDownKeyDown(event) {
        const menu = this.menu
        if(menu.orientation === 'vertical') {
            const items = this.menu.items.filter(({ disabled }) => !disabled)
            const index = items.indexOf(this) + 1
