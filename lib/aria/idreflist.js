...
function getObjectRef(object) {
    ...
    return RoleAttrAssembler.getInstanceOf(document.getElementById(object))
}
...
export class IDREFListAttrType extends ARIAAttrAssembler {
    ...
    set value(value) {
        if(this.constructor.removeOnValue(value)) {
            ...
            if(list.length) {
                this._refs = list.map(getObjectRef)
                this.node.value = list.map(getObjectId).join(IDREF_SEPARATOR)
            }
            else this.remove()
        }
        else {
