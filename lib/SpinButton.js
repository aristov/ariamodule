...
export class SpinButton extends Range {
    ...
    onDigitKeyDown(event) {
        const value = Number(this._edit.textContent.replace(MINUS_CHARACTER, '-') + event.key)
        this.valueNow = Math.min(Math.max(this.valueMin, value), this.valueMax)
    }
    ...
    onBackspaceKeyDown(event) {
        ...
        else if(Math.abs(valueNow) >= 10) {
            this.valueNow = Number(value.substring(0, value.length - 1))
        }
        else if(valueNow) {
            const valueMin = this.valueMin
            if(valueMin <= 0) {
    ...
    updateButtonState() {
        ...
        this._decrButton.setAttr(Disabled, disabled || valueNow !== null && valueNow <= this.valueMin)
        this._incrButton.setAttr(Disabled, disabled || valueNow !== null && valueNow >= this.valueMax)
    }
     ...
     */
    get step() {
        return this._step || this.constructor.defaultStep
    }
...
SpinButton.Box = Box
SpinButton.Edit = Edit
export const ARIASpinButton = Role.SpinButton = SpinButton
