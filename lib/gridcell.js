...
export class GridCell extends Cell {
    ...
    onKeyDown(event) {
        const key = event.key
        if(this.grabbed) this.grabbed = String(event.altKey)
        switch(key) {
    ...
    onBackspaceKeyDown(event) {
        if(!this.readOnly && this.mode !== 'edit') {
            ...
            if(cells.length) cells.forEach(cell => {
        ...
        if(event.target === this.node) {
            ...
            if(cell) {
                ...
                if(sibling) {
                    ...
                    else if(!this.readOnly && this.grabbed === 'true') this.replaceWith(sibling) // fixme: should be optional
                    else sibling.focus()
                }
            }
        }
