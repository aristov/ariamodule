...
export class Slider extends Range {
    ...
    set labelledBy(labelledBy) {
        ...
        this.findAll(this.constructor.Thumb).forEach(thumb => {
            thumb.labelledBy = labelledBy
        })
    }
     ...
     */
    get name() {
        return this.find(this.constructor.Thumb).name
    }
    ...
    set valueNow(valueNow) {
        const thumbs = this.findAll(this.constructor.Thumb)
...
class Thumb extends Range {
    ...
    set valueNow(valueNow) {
        ...
        // super.valueNow = this._input.value = Math.min(Math.max(min, valueNow), max) // todo
        super.valueNow = this._input.value = Math.min(Math.max(this.valueMin, valueNow), this.valueMax)
