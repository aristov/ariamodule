...
export class SpinButton extends ARIARange {
    ...
    onDigitKeyDown(event) {
        this.valueNow = Number(this._text.textContent.replace(MINUS_CHARACTER, '-') + event.key)
    }
     ...
     */
    onDotKeyDown(event) {
        if(/\./.test(this.step.toString()) && !/\./.test(this._text.textContent)) {
    ...
    onHyphenKeyDown(event) {
        ...
        if(!valueNow) {
            this._text.textContent = MINUS_CHARACTER
        }
    ...
    set valueNow(valueNow) {
        if(valueNow !== null) {
            ...
            this._text.textContent = String(value).replace(/-/, MINUS_CHARACTER)
        }
        else super.valueNow = this._text.textContent = this._input.value = null
    }
